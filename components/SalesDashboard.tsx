...
// Тестовые данные для случаев, когда API недоступен
const getMockData = () => {
  return [
    ["Ольга Ф.", "30000", "Смета"],
    ["Ольга Ф.", "300", "Смета"],
...
const SalesDashboard = () => {
  ...
  const fetchSheetData = async () => {
    try {
      // Для демонстрации в CodeSandbox используем тестовые данные
      return getMockData();

      // Если вы переносите код на свой сервер, раскомментируйте код ниже:
      /*
  ...
  const processSheetData = (rawData) => {
    ...
    const estimateCount = new Map();
    const amountByManager = new Map();
    ...
    transactions.forEach((t) => {
      ...
      amountByManager.get(t.name).push(t.amount);
    ...
    const highestAmountMap = new Map();
    ...
    const newChampions = {
      ...
      highestAmount: getMaxEntry(highestAmountMap),
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (rawData && rawData.length > 0) {
        ...
        if (
          processedData.transactions.length > lastTransactionCountRef.current
        ) {
          // Получаем последнюю транзакцию
          const latest =
            processedData.transactions[processedData.transactions.length - 1];
            ...
            >
              <div style={{ width: "350px" }}>
                ...
                >
                  ...
                  >
                    {formatNumber(champions.highestAmount.count)} €
                  </div>
                </div>
                <div
                  style={{
        ...
        >
          <div style={{ width: "500px" }}>
            <div style={{ display: "flex", marginBottom: "8px" }}>
              <div
                style={{ width: "170px", color: "#fb923c", fontWeight: "bold" }}
              >
                Менеджер
              </div>
              <div
            ...
            {summaryData.map((manager, i) => (
              <div key={i} style={{ display: "flex", marginBottom: "3px" }}>
                <div style={{ width: "170px", color: "#fb923c" }}>
                  {manager.manager}
                </div>
                <div
                  style={{
                ...
                >
                  {formatNumber(manager.totalAmount)} €
                </div>
                <div
                  style={{
                    width: "100px",
